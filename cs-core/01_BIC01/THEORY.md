...
### 2. Compilación
C++ es un lenguaje compilado. El código fuente (.cpp) se transforma en código objeto y luego en un ejecutable.
...
### 3. Variables y Tipos
- `int` (enteros), `float`/`double` (decimales), `char` (caracteres), `bool` (booleanos).
- C++ es un lenguaje de tipado estático (se debe declarar el tipo).

### 4. Control de Flujo
- **Condicionales**: `if`, `else if`, `else`, `switch`.
- **Bucles**: `for` (iteraciones definidas), `while` (mientras se cumpla condición), `do-while`.
